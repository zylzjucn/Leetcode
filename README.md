...
Union find:
1. 有个问题要注意，新出现的点，第一种情况是只可能造成group-1，这种好办，遇到一个就总数-1，第二种可能 -2，-3，不好弄，要注意最后数数的方法
2. 还有一个要注意，一定要初始化指向自己，在find中做这件事情也可以。尤其在`unordered_map`的时候
3. 切记，做后比较的时候，要在调用`find`，而不是直接比较`leaders`里面两者的值。因为只有这张，我们才能最后再压缩一次路径，保证压倒最扁平，也就是root节点永远指向自己。因为之前的更新可能会破坏这一点。
